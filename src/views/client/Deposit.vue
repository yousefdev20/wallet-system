            ...
            <div class="row">
              <div class="col-md-6 mb-md-0 mb-4">
                ...
                <div class="card card-body border card-plain border-radius-lg d-flex align-items-center flex-row rounded-0">
                  <select class="form-control w-100 pt-0 pb-0 mb-0 mt-0" v-model="account.payment_gateway_id" @change="currency">
              ...
              <div class="col-md-6 mb-md-0 mb-4" :class="{'disable': !allow_currency}">
                <div class="col-6 d-flex align-items-center">
                  <h6 class="mb-0">Currency</h6>
                </div>
                <div class="card card-body border card-plain border-radius-lg d-flex align-items-center flex-row rounded-0">
                  <select class="form-control w-100 pt-0 pb-0 mb-0 mt-0" v-model="account.currency_id">
                    <option v-for="(item, index) in currencies" :value="item.id" :key="index">{{ item.code }}</option>
                  </select>
                </div>
                <b v-if="errors.currency_id" class="text-sm text-danger mt-0 pt-0">{{ errors.currency_id.join(', ') }}</b>
              </div>
...
export default {
  ...
  methods: {
    ...
    currency () {
      this.allow_currency = true
      this.account.is_multi_currency = true
      if (this.payment_gateways[this.account.payment_gateway_id].allow_currency){
        this.account.is_multi_currency = false
        this.allow_currency = false
      }
    },
    add_transaction () {
      this.reset_error_message()
      this.axios.post('deposit', this.account).then(response => {
